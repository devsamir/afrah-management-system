...
import { useBiayaPenjahitan } from "../../hooks/Sarung/biayaPenjahitanHooks";
...
const columns: Columns[] = [
  ...
  { field: "namaPenjahit", headerName: "Nama Penjahit" },
  {
    field: "jumlahSarung",
...
const BiayaPenjahitan = () => {
  const State = useBiayaPenjahitan();
  ...
  const [penjahit, setPenjahit] = useState<any>({});
  ...
  const [penjahitP, setPenjahitP] = useState("");
  // Input Piutang
  ...
  const handleUpdate = async () => {
    ...
    if (one) {
      ...
      setPenjahit({ value: one.idPenjahit, label: one.namaPenjahit });
  ...
  const handleGetPiutang = async () => {
    ...
    if (piutang) {
      setPenjahitP(piutang.penjahitan.namaPenjahit);
      setTanggalP(piutang.penjahitan.tanggal);
      setHargaP(piutang.penjahitan.harga);
      setJumlahSarungP(piutang.penjahitan.jumlahSarung);
      setTotalP(piutang.penjahitan.total);
      setTagihanP(piutang.penjahitan.sisaTagihan);
      setKeteranganP(piutang.penjahitan.keterangan);
  ...
  return (
    <>
            ...
            <div style={{ width: 200 }} className="mx-2">
              ...
              <Select
                ...
                options={[
                  ...
                  ...State.penjahitFilter.map((item) => ({
                    value: item.id,
                    label: item.namaPenjahit,
                  })),
                ]}
        ...
        <form onSubmit={handleSubmit}>
          <Modal.Header>
            <Modal.Title className="text-capitalize">
              {inputStatus} biaya penjahitan sarung
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {inputStatus === "update" && (
              <span className="text-danger">
                * Mengupdate Data Penjahitan Akan Menghapus Semua Transaksi
                Pembayaran Piutang
              </span>
            )}
            <div className="form-group my-2">
              ...
              <Select
                value={penjahit}
                options={State.penjahitAdd.map((item) => ({
                  value: item.id,
                  label: item.namaPenjahit,
                }))}
                onChange={(value) => setPenjahit(value)}
              />
            </div>
            <div className="form-group my-2">
              <label>
                Tanggal Penjahitan<span className="text-danger">*</span>
              </label>
              <input
                className="form-control"
                type="date"
      ...
      >
        <Modal.Header>
          <Modal.Title className="text-capitalize">
            piutang biaya penjahitan sarung
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="row">
            <div className="col-6">
              <label>Penjahit</label>
              <input type="text" className="form-control" value={penjahitP} />
            </div>
            <div className="col-6">
              <label>Tanggal</label>
      ...
      >
        <form onSubmit={handleSubmitPiutang}>
          <Modal.Header>
            <Modal.Title className="text-capitalize">
              {inputStatusP} piutang biaya penjahitan sarung
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="form-group my-2">
...
export default BiayaPenjahitan;
